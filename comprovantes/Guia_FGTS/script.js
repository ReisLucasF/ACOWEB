async function generatePDF() {
  ...
  const convenioMatch = textInput.match(/Codigo do Convenio\s*:\s*(\d+)/i);
  const convenio = convenioMatch ? convenioMatch[1] : '';

  let convenioformatado = '';

  const conveniosValidos = ["12173", "12157", "12190", "1221", "12513"];

  if (conveniosValidos.includes(convenio)) {
      convenioformatado = "0180 - com centralização ou com tomador";
  } else if (convenio === "00116") {
      convenioformatado = "418 - FGTS / GFIP";
  } else if (convenio === "13617") {
      convenioformatado = "411 - FGTS / GFIP";
  } else if (convenio === "14010") {
      convenioformatado = "412 - FGTS / GFIP";
  } else {
      // ...
  }

  console.log(convenioformatado);

  let dataCompetenciaFormatada = ''

  const competenciaMatch = textInput.match(/Data de competencia\s*:\s*(\d{2})(\d{4})/i);
  if (competenciaMatch) {
      const mes = competenciaMatch[1];
      const ano = competenciaMatch[2];
      dataCompetenciaFormatada = `${mes}/${ano}`;
      console.log(dataCompetenciaFormatada); // Saída: 05/2018
  } else {
      console.log("Data de competência não encontrada.");
  }


  let valorJurosMulta = ''
  const jurosMultaMatch = textInput.match(/Valor dos juros\/multa\s*:\s*R\$\s*([\d,\.]+)/i);
  if (jurosMultaMatch) {
      valorJurosMulta = jurosMultaMatch[1];
      console.log(`Valor dos juros/multa: R$ ${valorJurosMulta}`); // Saída: Valor dos juros/multa: R$ 0,00
  } else {
      console.log("Valor dos juros/multa não encontrado.");
  }
...
const htmlContent = await response.text();
  ...
  .replace('<td id="competencia"></td>', `<td class="foco" id="competencia">${dataCompetenciaFormatada}</td>`)
  .replace('<td id="valorDocumento"></td>', `<td class="foco" id="valorDocumento">${valorDocumentoFormatado}</td>`)
  .replace('<td id="jurosMulta"></td>', `<td class="foco" id="jurosMulta">${valorJurosMulta}</td>`)
  ...
  .replace('<td id="convenio"></td>', `<td class="foco" id="convenio">${convenioformatado}</td>`)
